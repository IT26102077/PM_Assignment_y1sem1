 ...
 * window [thetaMinDeg, thetaMaxDeg]. Same idea as escort_attack_range's
 * angle clamp: fire at whichever angle inside the window is closest to
 * 45 degrees (minimises the speed needed to reach any given distance),
 * then solve for the speed required to reach the target distance exactly.
 *
 * Part 1-A/B "Simulation 1" call this with [0,90] (B's full, un-jammed
 * range). Part 1-B "Simulation 2" calls it with [thetaMin, 90] once the
 * gun has jammed.
 */
